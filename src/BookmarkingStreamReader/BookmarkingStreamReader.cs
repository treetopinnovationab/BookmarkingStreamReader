#undef STREAMREADERSUPPORTSASYNC
...
namespace Bookmarking {
    ...
    public class BookmarkingStreamReader : BookmarkingTextReader {
        ...
        public string DebugState() {
            return _currentPositionDetails.GetDebugState();
        }
        ...
        protected override int _Read(char[] buffer, int index, int count)
        {
            if (buffer == null)
            {
        ...
        protected override int _Read(Span<char> buffer) =>
            GetType() == typeof(BookmarkingStreamReader) ? ReadSpan(buffer) :
            base._Read(buffer); // Defer to Read(char[], ...) if a derived type may have previously overridden it
        ...
        protected override int _ReadBlock(char[] buffer, int index, int count)
        {
            if (buffer == null)
            {
                throw new ArgumentNullException(nameof(buffer), SR.ArgumentNull_Buffer);
        ...
        protected override int _ReadBlock(Span<char> buffer)
        {
            if (GetType() != typeof(BookmarkingStreamReader))
            {
                // Defer to Read(char[], ...) if a derived type may have previously overridden it.
                return base._ReadBlock(buffer);
            }
        ...
        private bool IsPreamble()
        {
            ...
            if (_checkPreamble)
            {
                if (_bytePos == preamble.Length)
                {
                    ...
                    _currentPositionDetails.MovedPastPreambleOfByteLength(preamble.Length);
                    _bytePos = 0;
                    _checkPreamble = false;
                    _detectEncoding = false;
                }
        ...
        protected virtual int ReadBuffer()
        {
            ...
            do
            {
                ...
                var charLenNew = _decoder.GetChars(_byteBuffer, 0, _byteLen, _charBuffer, _charLen);
                var justNewChars = new char[charLenNew];
                Array.Copy(_charBuffer, _charLen, justNewChars, 0, charLenNew);
                _charLen += charLenNew;
                _currentPositionDetails.ReadBytesAndChars(byteCount:_byteLen, charCount:charLenNew, bytes:_byteBuffer, encoding:_encoding, chars:justNewChars);
            } while (_charLen == 0);
        ...
        public void ResumeFromBookmark(LineBookmark lineBookmark) {
            DiscardBufferedData();
            var maxPosition = _stream.Length;
            if (lineBookmark.Position > maxPosition) {
                ...
                _stream.Seek(lineBookmark.Position+1, SeekOrigin.Begin);
        ...
        public ReadDetailedLineResult? ReadDetailedLine()
        {
            ...
            var startLineBytePos = _bytePos;
            var startLineCharPos = _charPos;
            ...
            do
            {
                ...
                do
                {
                    ...
                    if (ch == '\r' || ch == '\n')
                    {
                        ...
                        var posBeforeCharPos = _charPos - 1;
                ...
                sb.Append(_charBuffer, _charPos, i);
                furthestCharIndex = (_charPos + i) - 1;
            } while (ReadBuffer() > 0);
        ...
        #endregion
#endif // STREAMREADERSUPPORTSASYNC
...
#endif // STREAMREADERSUPPORTSNULLREADER
    }
