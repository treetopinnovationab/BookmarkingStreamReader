...
namespace Bookmarking {
    ...
    public abstract partial class BookmarkingTextReader : MarshalByRefObject, IDisposable {
        ...
        //
        protected virtual int _Read(char[] buffer, int index, int count)
        {
            if (buffer == null)
            {
        ...
        //
        protected virtual int _Read(Span<char> buffer)
        {
            char[] array = ArrayPool<char>.Shared.Rent(buffer.Length);

            try
            {
                int numRead = _Read(array, 0, buffer.Length);
        ...
        // 
        protected virtual int _ReadBlock(char[] buffer, int index, int count)
        {
            int i, n = 0;
            do
            {
        ...
        // characters have been read or the end of the file was reached.
        //
        protected virtual int _ReadBlock(Span<char> buffer)
        {
            char[] array = ArrayPool<char>.Shared.Rent(buffer.Length);

            try
            {
                int numRead = _ReadBlock(array, 0, buffer.Length);
                if ((uint)numRead > (uint)buffer.Length)
                {
                    throw new IOException(SR.IO_InvalidReadLength);
                }
...
#if TEXTREADERSUPPORTSASYNC
#region Task based Async APIs
        public virtual Task<string> ReadLineAsync() {
            return Task<string>.Factory.StartNew(state => {
                return ((BookmarkingTextReader)state).ReadLine();
            },
        ...
        public virtual ValueTask<int> ReadAsync(Memory<char> buffer, CancellationToken cancellationToken = default) =>
            new ValueTask<int>(MemoryMarshal.TryGetArray(buffer, out ArraySegment<char> array) ?
                ...
                Task<int>.Factory.StartNew(state => {
                    var t = (Tuple<BookmarkingTextReader, Memory<char>>)state;
        ...
        internal virtual ValueTask<int> ReadAsyncInternal(Memory<char> buffer, CancellationToken cancellationToken) {
            var tuple = new Tuple<BookmarkingTextReader, Memory<char>>(this, buffer);
        ...
        public static BookmarkingTextReader Synchronized(BookmarkingTextReader reader)
        {
            ...
            return reader is SyncBookmarkingTextReader ? reader : new SyncBookmarkingTextReader(reader);
        }

        internal sealed class SyncBookmarkingTextReader : BookmarkingTextReader {
            internal readonly BookmarkingTextReader _in;
...
#endif // TEXTREADERSUPPORTSSYNCHRONIZED
    }
}