...
#undef STREAMREADERSUPPORTSNULLREADER
#undef STREAMREADERSUPPORTSSYNCHRONIZED
...
namespace Bookmarking {
    ...
    public class BookmarkingStreamReader : BookmarkingTextReader {
#if STREAMREADERSUPPORTSNULLREADER
        // BookmarkingStreamReader.Null is threadsafe.
        public new static readonly BookmarkingStreamReader Null = new NullBookmarkingStreamReader();
#endif
        ...
        internal static int DefaultBufferSize {
            get {
#if QUASI_FEATURE_LEGACYNETCF
                // Quirk for Mango app compatibility
                if (CompatibilitySwitches.IsAppEarlierThanWindowsPhone8)
                {
                    return 4096;
                }
#endif // QUASI_FEATURE_LEGACYNETCF
                return 1024;
            }
        }
        ...
        // disposing of this BookmarkingStreamReader.  A name like _leaveOpen is better, 
...
#if QUASI_FEATURE_ASYNC_IO
        // We don't guarantee thread safety on BookmarkingStreamReader, but we should at 
        ...
        // BookmarkingStreamReader by default will ignore illegal UTF8 characters. We don't want to 
        // throw here because we want to be able to read ill-formed data without choking. 
        // The high level goal is to be tolerant of encoding errors when we read and very strict 
        // when we write. Hence, default StreamWriter encoding will throw on error.   

        internal BookmarkingStreamReader() {
        }

        public BookmarkingStreamReader(Stream stream)
            : this(stream, true) {
        }

        public BookmarkingStreamReader(Stream stream, bool detectEncodingFromByteOrderMarks)
            : this(stream, Encoding.UTF8, detectEncodingFromByteOrderMarks, DefaultBufferSize, false) {
        }

        public BookmarkingStreamReader(Stream stream, Encoding encoding)
            : this(stream, encoding, true, DefaultBufferSize, false) {
        }

        public BookmarkingStreamReader(Stream stream, Encoding encoding, bool detectEncodingFromByteOrderMarks)
            : this(stream, encoding, detectEncodingFromByteOrderMarks, DefaultBufferSize, false) {
        }

        // Creates a new BookmarkingStreamReader for the given stream.  The 
        ...
        // 
        public BookmarkingStreamReader(Stream stream, Encoding encoding, bool detectEncodingFromByteOrderMarks, int bufferSize)
            : this(stream, encoding, detectEncodingFromByteOrderMarks, bufferSize, false) {
        }

        public BookmarkingStreamReader(Stream stream, Encoding encoding, bool detectEncodingFromByteOrderMarks, int bufferSize, bool leaveOpen) {
        ...
        public BookmarkingStreamReader(String path)
            : this(path, true) {
#if QUASI_FEATURE_LEGACYNETCF
            if(CompatibilitySwitches.IsAppEarlierThanWindowsPhone8) {
        ...
        [ResourceExposure(ResourceScope.Machine)]
        [ResourceConsumption(ResourceScope.Machine)]
        public BookmarkingStreamReader(String path, bool detectEncodingFromByteOrderMarks)
            : this(path, Encoding.UTF8, detectEncodingFromByteOrderMarks, DefaultBufferSize) {
        }

        [ResourceExposure(ResourceScope.Machine)]
        [ResourceConsumption(ResourceScope.Machine)]
        public BookmarkingStreamReader(String path, Encoding encoding)
            : this(path, encoding, true, DefaultBufferSize) {
        }

        [ResourceExposure(ResourceScope.Machine)]
        [ResourceConsumption(ResourceScope.Machine)]
        public BookmarkingStreamReader(String path, Encoding encoding, bool detectEncodingFromByteOrderMarks)
        ...
        [ResourceExposure(ResourceScope.Machine)]
        [ResourceConsumption(ResourceScope.Machine)]
        public BookmarkingStreamReader(String path, Encoding encoding, bool detectEncodingFromByteOrderMarks, int bufferSize) {
        ...
        [ResourceExposure(ResourceScope.Machine)]
        [ResourceConsumption(ResourceScope.Machine)]
        internal BookmarkingStreamReader(String path, Encoding encoding, bool detectEncodingFromByteOrderMarks, int bufferSize, bool checkHost) {
        ...
        // Init used by NullBookmarkingStreamReader, to delay load encoding
        ...
        protected override void Dispose(bool disposing) {
            // Dispose of our resources if this BookmarkingStreamReader is closable.
        ...
        // DiscardBufferedData tells BookmarkingStreamReader to throw away its internal
        // buffer contents.  This is useful if the user needs to seek on the
        // underlying stream to a known location then wants the BookmarkingStreamReader
        // to start reading from this new point.  This method should be called
        // very sparingly, if ever, since it can lead to very poor performance.
        // However, it may be the only way of handling some scenarios where 
        // users need to re-read the contents of a BookmarkingStreamReader a second time.
        public void DiscardBufferedData() {
            ...
            _currentPositionDetails.ForgetState();
            _previousLineBookmark = null;

            byteLen = 0;
            charLen = 0;
            charPos = 0;
            // in general we'd like to have an invariant that encoding isn't null. However,
            // for startup improvements for NullBookmarkingStreamReader, we want to delay load encoding. 
            if (encoding != null) {
                decoder = encoding.GetDecoder();
            }
            _isBlocked = false;
        ...
        protected override int _Read([In, Out] char[] buffer, int index, int count) {
            if (buffer == null)
        ...
        protected override String _ReadToEnd() {
            if (stream == null)
                __Error.ReaderClosed();
        ...
        protected override int _ReadBlock([In, Out] char[] buffer, int index, int count) {
            if (buffer == null)
                throw new ArgumentNullException("buffer", SR.ArgumentNull_Buffer);
            if (index < 0 || count < 0)
                throw new ArgumentOutOfRangeException((index < 0 ? "index" : "count"), SR.ArgumentOutOfRange_NeedNonNegNum);
            ...
            return base._ReadBlock(buffer, index, count);
        }

        // Trims n bytes from the front of the buffer.
        private void CompressBuffer(int n) {
            Contract.Assert(byteLen >= n, "CompressBuffer was called with a number of bytes greater than the current buffer length.  Are two threads using this BookmarkingStreamReader at the same time?");
        ...
        private bool IsPreamble() {
            ...
            Contract.Assert(bytePos <= _preamble.Length, "_compressPreamble was called with the current bytePos greater than the preamble buffer length.  Are two threads using this BookmarkingStreamReader at the same time?");
            ...
            Contract.Assert(bytePos <= _preamble.Length, "possible bug in _compressPreamble.  Are two threads using this BookmarkingStreamReader at the same time?");

            if (_checkPreamble) {
                if (bytePos == _preamble.Length) {
                    // We have a match
                    CompressBuffer(_preamble.Length);
                    _currentPositionDetails.MovedPastPreambleOfByteLength(_preamble.Length);
                    bytePos = 0;
                    _checkPreamble = false;
                    _detectEncoding = false;
                }
        ...
        public void SetCurrentPositionDebug(Action<string> debugAction) {
            _currentPositionDetails.SetDebug(debugAction);
        }

        public void LocallyTrackDebug() {
            SetCurrentPositionDebug(s => _localDebug.AppendLine($"{DateTime.UtcNow:yyyy-MM-dd HH:mm:ss.ffff} {s}"));
        }

        private readonly StringBuilder _localDebug = new StringBuilder();

        public string LocalDebugText => _localDebug.ToString();

        private readonly BookmarkingCurrentPositionDetails _currentPositionDetails = new BookmarkingCurrentPositionDetails();
        ...
        internal virtual int ReadBuffer() {
            ...
            do {
                if (_checkPreamble) {
                    ...
                    Contract.Assert(bytePos == 0, "bytePos can be non zero only when we are trying to _checkPreamble.  Are two threads using this BookmarkingStreamReader at the same time?");
                ...
                var charLenNew = decoder.GetChars(byteBuffer, 0, byteLen, charBuffer, charLen);
                var justNewChars = new char[charLenNew];
                Array.Copy(charBuffer, charLen, justNewChars, 0, charLenNew);
                charLen += charLenNew;
                _currentPositionDetails.ReadBytesAndChars(byteCount: byteLen, charCount: charLenNew, bytes: byteBuffer, encoding: encoding, chars: justNewChars);
        ...
        // user's buffer, bypassing BookmarkingStreamReader's own buffer.
                    ...
                    if (len == 0) {
                        ...
                        if (byteLen > 0) {
                            if (readToUserBuffer) {
                                ...
                                charLen = 0;  // BookmarkingStreamReader's buffer is empty.
                            } else {
                                charsRead = decoder.GetChars(byteBuffer, 0, byteLen, charBuffer, charsRead);
                                charLen += charsRead;  // Number of chars in BookmarkingStreamReader's buffer.
                            }
                        }
        ...
        //
        protected override String _ReadLine() {
            if (stream == null)
                __Error.ReaderClosed();
        ...
        public void ResumeFromBookmark(LineBookmark lineBookmark) {
            DiscardBufferedData();
            var maxPosition = stream.Length;
            if (lineBookmark.Position > maxPosition) {
                stream.Seek(0, SeekOrigin.End);
                _currentPositionDetails.MovedToPosition(maxPosition, lineBookmark.CharIndex + 1);
                _previousLineBookmark = lineBookmark;
            } else {
                stream.Seek(lineBookmark.Position + 1, SeekOrigin.Begin);
                _currentPositionDetails.MovedToPosition(lineBookmark.Position + 1, lineBookmark.CharIndex + 1);
                _previousLineBookmark = lineBookmark;
            }
        ...
        public void ResumeFromBeginning() {
            DiscardBufferedData();
            {
                ...
                _currentPositionDetails.MovedToPosition(0, 0);
                _previousLineBookmark = null;
            }

        }

        public LineBookmark MakeCurrentPositionBookmark() {
            var bytePos = _currentPositionDetails.GetBytePositionOfStartOfCurrentBuffer() - 1;
            var charPos = _currentPositionDetails.GetCharPositionOfStartOfCurrentBuffer() - 1;
            return new LineBookmark(bytePos, charPos);
        }

        private LineBookmark? _previousLineBookmark = null;
        ...
        // value is null if the end of the input stream has been reached.
        //
        public ReadDetailedLineResult? ReadDetailedLine() {
            if (stream == null)
                __Error.ReaderClosed();
            ...
            var beforeReadingLineBookmark = _previousLineBookmark ?? MakeCurrentPositionBookmark();

            if (charPos == charLen) {
                if (ReadBuffer() == 0) return null;
            }

            var lineEnding = BookmarkingLineEnding.None;

            var startLineBytePos = bytePos;
            var startLineCharPos = charPos;
            var absStartLineBytePos = _currentPositionDetails.AbsoluteBytePositionOfCharIndexInCurrentBuffer(startLineCharPos);
            var absStartLineCharPos = _currentPositionDetails.AbsoluteCharPositionOfCharIndexInCurrentBuffer(startLineCharPos);


            var furthestCharIndex = -1;
            ...
            do {
                ...
                do {
                    ...
                    if (ch == '\r' || ch == '\n') {
                        ...
                        charPos = i + 1;
                        if (ch == '\n') {
                            lineEnding = BookmarkingLineEnding.LineFeed;
                        }
                        if (ch == '\r') {
                            lineEnding = BookmarkingLineEnding.CarriageReturn;
                        }
                        if (ch == '\r' && (charPos < charLen || ReadBuffer() > 0)) {
                            if (charBuffer[charPos] == '\n') {
                                lineEnding = BookmarkingLineEnding.CarriageReturnLineFeed;
                                charPos++;
                            }
                        }

                        var posBeforeCharPos = charPos - 1;

                        var absEndLineBytePos = _currentPositionDetails.AbsoluteBytePositionOfCharIndexInCurrentBuffer(posBeforeCharPos);
                        var absEndLineCharPos = _currentPositionDetails.AbsoluteCharPositionOfCharIndexInCurrentBuffer(posBeforeCharPos);

                        var result = new ReadDetailedLineResult(s, lineEnding, absStartLineBytePos, absEndLineBytePos, absEndLineCharPos, beforeReadingLineBookmark);
                        _previousLineBookmark = result.MakeBookmarkForReadingNextLine();
                        return result;
                    }
                    i++;
                } while (i < charLen);
                ...
                if (sb == null) sb = new StringBuilder(i + 80);
                sb.Append(charBuffer, charPos, i);
                furthestCharIndex = (charPos + i) - 1;
            } while (ReadBuffer() > 0);

            var absEndLineBytePos2 = _currentPositionDetails.AbsoluteBytePositionOfCharIndexInCurrentBuffer(furthestCharIndex);
            var absEndLineCharPos2 = _currentPositionDetails.AbsoluteCharPositionOfCharIndexInCurrentBuffer(furthestCharIndex);
            var res = new ReadDetailedLineResult(sb.ToString(), BookmarkingLineEnding.None, absStartLineBytePos, absEndLineBytePos2, absEndLineCharPos2, beforeReadingLineBookmark);
            _previousLineBookmark = res.MakeBookmarkForReadingNextLine();
            return res;
        }
...
#region Task based Async APIs
        ...
        public override Task<String> ReadLineAsync()
        {
            ...
            if (this.GetType() != typeof(BookmarkingStreamReader))
            ...
            while (count > 0)
            {
                ...
                if (n == 0)
                {
                    ...
                    do
                    {
                        ...
                        if (CheckPreamble_Prop)
                        {
                            Contract.Assert(BytePos_Prop <= Preamble_Prop.Length, "possible bug in _compressPreamble.  Are two threads using this BookmarkingStreamReader at the same time?");
                            ...
                            if (len == 0)
                            {
                                ...
                                if (ByteLen_Prop > 0)
                                {
                                    if (readToUserBuffer)
                                    {
                                        ...
                                        CharLen_Prop = 0;  // BookmarkingStreamReader's buffer is empty.
                                    }
                                    else
                                    {
                                        n = Decoder_Prop.GetChars(tmpByteBuffer, 0, ByteLen_Prop, CharBuffer_Prop, 0);
                                        CharLen_Prop += n;  // Number of chars in BookmarkingStreamReader's buffer.
                                    }
                                }
                        ...
                        {
                            Contract.Assert(BytePos_Prop == 0, "_bytePos can be non zero only when we are trying to _checkPreamble.  Are two threads using this BookmarkingStreamReader at the same time?");
...
#region Private properties for async method performance
        ...
        private class NullBookmarkingStreamReader : BookmarkingStreamReader {
            // Instantiating Encoding causes unnecessary perf hit. 
            internal NullBookmarkingStreamReader() {
                Init(Stream.Null);
            }
            ...
            protected override int Read(char[] buffer, int index, int count) {
                return 0;
            }
